...
export function processUploadedImage(file: File): Promise<ImageProcessResult> {
  return new Promise((resolve) => {
    if (!file.type.startsWith('image/')) {
      resolve({ dataUrl: null, error: 'Ese archivo no es una imagen. Elige una foto (JPG, PNG, etc.).' });
      return;
    }
    if (file.size > MAX_ORIGINAL_SIZE_MB * 1024 * 1024) {
      resolve({ dataUrl: null, error: `La foto es muy pesada (máximo ${MAX_ORIGINAL_SIZE_MB}MB).` });
      return;
    }

    const reader = new FileReader();
    reader.onerror = () => resolve({ dataUrl: null, error: 'No se pudo leer la foto. Intenta de nuevo.' });
    reader.onload = () => {
      const img = new Image();
      img.onerror = () => resolve({ dataUrl: null, error: 'No se pudo procesar la foto. Intenta con otra.' });
      img.onload = () => {
        ...
        if (!ctx) {
          resolve({ dataUrl: null, error: 'Tu navegador no permite procesar imágenes.' });
          return;
        }
